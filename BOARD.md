# Board

Лёгкая Kanban-альтернатива на одном markdown-файле: три колонки
(To Do / Doing / Done) под git, без внешних сервисов и инструментов.

## Соотношение с другими файлами

- `BACKLOG.md` — длинная очередь идей и побочных находок. Сюда падает
  «потом подумаем», «не сейчас». Парковка scope.
- `BOARD.md` (этот файл) — активный рабочий поток. Задачи, которые мы
  уже взяли или собираемся брать в ближайшее время.
- `specs/T<NNN>-*/spec.md` — куда вырастает крупная задача из BOARD, если
  она оказывается фичей >1 дня работы.

Жизненный цикл задачи: идея в `BACKLOG.md` → созрела → переезжает в
`To Do` здесь → берётся в работу (`Doing`) → закрывается (`Done`) →
после релиза переходит в `CHANGELOG.md` (запись обязательно содержит
T-ID), отсюда удаляется. **`CHANGELOG.md` — единственное persistent-
хранилище T-ID завершённых задач**, без него правило «ID не
переиспользуется» сломается.

## Формат задачи

Каждая задача — `- **T<NNN>** — <короткое описание>`. ID присваивается
при создании: новый = `max(существующих T-ID в BOARD.md, BACKLOG.md и
CHANGELOG.md) + 1`. ID никогда не переиспользуется. ID общий для
`BOARD.md` и `BACKLOG.md` — при перетекании задачи между ними
сохраняется; после релиза задача попадает в `CHANGELOG.md` с тем же
T-ID, что гарантирует уникальность номеров между релизами.

Имя ветки: `T<NNN>-<slug>` (без namespace типа `fixes/` / `feature/` —
ID уже даёт идентификацию). Имя PR: `T<NNN>: <title>`. Спецификация
крупной фичи: `specs/T<NNN>-<slug>/spec.md`.

По вкусу можно добавлять:

- метку даты взятия,
- ссылку на спеку,
- имя ветки.

Пример:

```
- **T<NNN>** — Превью постов в Telegram
  (`specs/T<NNN>-telegram-preview/`, ветка `T<NNN>-telegram-preview`).
```

---

## To Do

<!-- Готово к взятию. Очередь FIFO по умолчанию, можно поднимать
     приоритетное наверх. -->

<!-- Записи задач в формате `- **T<NNN>** — описание`. См. раздел
     «Формат задачи» выше. -->



## Doing

<!-- В работе прямо сейчас. Держим короткой: максимум 1-2 задачи на
     разработчика, иначе теряется фокус (классическое WIP-limit
     правило из Kanban). -->



## Done

- **T013** — Пропорции окна от шрифта платформы; компактный вид на
  Windows [closed 2026-08-31].

- **T012** — Полировка окна: системный питон (шрифты), zenity-диалоги
  (согласованный noqa S603), одна кнопка, ширина/отступы; принято
  Разработчиком глазами [closed 2026-08-31].

- **T011** — Простое окно (tkinter): параметры, файл/папка, лог,
  память настроек; единый exe (окно по клику, CLI при аргументах)
  [closed 2026-08-31].

- **T010** — Дефолты в пунктах: красная 3 pt, синяя 2 pt, вплотную;
  раздельные толщины обводок [closed 2026-08-31].

- **T009** — Выход = все исходные слои + новый слой только с линиями
  (откат удаления из T008; no-fill остался) [closed 2026-08-31].

- **T008** — Результат — только линии: кривые слоя зоны удаляются,
  явная fild-запись «нет заливки»; проверено в живом CorelDRAW 26
  [closed 2026-08-31].

- **T007** — Потоковая укладка data-файлов (порядок дерева); корень
  «не открывается» найден и закрыт, проверено в живом CorelDRAW 26
  на стендовой VM [closed 2026-08-31].

- **T006** — Родная раскладка loda: сентинел и данные после таблиц;
  идемпотентность закреплена тестом на реальных байтах
  [closed 2026-08-30].

- **T005** — Детект отсутствия залитой зоны: зона ищется только среди
  залитых кривых; без неё файл пропускается вслух [closed 2026-08-30].

- **T004** — Пакетный режим по умолчанию: без аргументов — все .cdr
  текущей папки, кроме *-lines.cdr [closed 2026-08-30].

<!-- Закрытые задачи, ждущие переноса в CHANGELOG.md при следующем
     релизе или значимой точке. После переноса — очищаем. -->

- **T003** — Аддитивный режим: двойная линия — новым слоем, исходные
  слои нетронуты; классификация только в слое зоны [closed 2026-08-30].
- **T002** — Репозиторий на GitHub, CI (гейты + windows-exe со смоуком),
  синтетика вынесена в пакет [closed 2026-08-30].
- **T001** — Преобразование залитой области в двойную красно-синюю линию
  [closed 2026-08-30; приёмка в настоящем CorelDRAW — на стороне
  заказчика].
