...
- **T003** — Не трогать чужие слои: классификация только внутри группы
  с залитой зоной, объекты вне её переносятся в выход нетронутыми.
  Acceptance criteria: кривые вне группы зоны не получают двойных линий
  и не удаляются; содержимое группы зоны, не являющееся кривой,
  сохраняется; тест на синтетике с посторонним слоем.
